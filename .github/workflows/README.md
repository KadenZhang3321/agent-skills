...
| 认证配置 | **API Key 模式**：`CLAUDE_API_KEY` 已由组织级 Secret 统一提供，无需目标仓库单独配置；**OIDC 模式**：需额外配置 AWS IAM OIDC 信任策略 |
...
with:
  ...
  auth_method: 'api_key'            # 认证方式：api_key 或 oidc
...
**OIDC 模式**（可选，替代 API Key）：

| 配置项 | 说明 |
|--------|------|
| AWS IAM Role | 在 AWS 控制台配置 GitHub OIDC 信任策略 |
| `aws_role_arn` | 在 workflow `with` 中填写 Role ARN |
...
```json
{
  ...
  "Condition": {
    "StringLike": {
      "token.actions.githubusercontent.com:sub": "repo:vllm-project/vllm-ascend:*"
    }
  }
}
```
...
**问题：OIDC 认证失败**
- 检查 AWS IAM Role 的信任策略是否包含正确的仓库路径
- 确认 `aws_role_arn` 和 `aws_region` 配置正确
- 确保 workflow 声明了 `id-token: write` 权限
